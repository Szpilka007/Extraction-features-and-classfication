...
public class XmlExtractor {
    ...
    public Resource[] loadFileFromResources(String classpath) throws IOException {
// "reuters-machine-learning\\reuters21578\\reut2-002.sgm"
        URL url = ResourceLoader.class.getResource("classpath:" + classpath);
        if (url != null) {
...
//          File file = resource.getFile();
            ...
            throw new IOException("The path was wrong or the file does not exist!");
        }
    }

    public void extractFileToDatabase(Resource sgmFile) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(ClassLoader.class.getResourceAsStream(sgmFile.toString()), StandardCharsets.ISO_8859_1))) {
    ...
    public ReutersEntity prepareReuters(String rawText) {
        ...
        if (!dateInText.isEmpty()) {
            date = LocalDateTime.parse(dateInText.stream().findFirst().get().trim(), dtFormatter);
        }
